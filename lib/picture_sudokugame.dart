// 2차원 배열로 문제 구성하는 거 안 했음!! result가 사진에서 추출한 2차원 배열
...
class PictureSudokuGame extends StatefulWidget {
  final String? result;

  const PictureSudokuGame({Key? key, this.result}) : super(key: key);
  // const PictureSudokuGame({super.key});

  @override
  _PictureSudokuGameState createState() => _PictureSudokuGameState();
}

class _PictureSudokuGameState extends State<PictureSudokuGame> {
  ...
  void initState() {
    ...
    fetchSudokuData();
    print("디버깅용 result: ${widget.result}");
  }
...
class SudokuApp extends StatelessWidget {
  ...
  const SudokuApp({required this.result, Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      home: PictureSudokuGame(result: result),
    );
  }
}
